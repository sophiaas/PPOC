...
from lightbot_env.maps import puzzles
from lightbot_env.maps.map_parsing import extract_map_features
# import torch
...
for k, v in env_dict.items():
    register(
        id=v,
        entry_point='lightbot_env:LightbotEnv',
        kwargs={
            'puzzle_name': k
        }
    )
...
class LightbotEnv(gym.Env):
    ...
    def __init__(self, puzzle_name):
        ...
        self.observation_space_gym = spaces.Dict({
...
#         if action in possible_actions:
        ...
        if self.frame_count >= self.max_count:
            ...
            self.death = True
        return copy.deepcopy(self.state), copy.deepcopy(self.reward), copy.deepcopy(self.done), self.death
