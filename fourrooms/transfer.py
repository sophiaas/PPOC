...
import sys
sys.path.append('..')  # for importing the env
from env import LB
...
if __name__ == '__main__':
    ...
    # env = gym.make('Fourrooms-v0')  # CHANGED
    env = LB('cross')  # CHANGED

    fname = '-'.join(['{}_{}'.format(param, val) for param, val in sorted(vars(args).items())])
    # fname = 'optioncritic-fourrooms-' + fname + '.npy'  # CHANGED
    fname = 'optioncritic-lightbot-' + fname + '.npy'  # CHANGED

    possible_next_goals = [68, 69, 70, 71, 72, 78, 79, 80, 81, 82, 88, 89, 90, 91, 92, 93, 99, 100, 101, 102, 103]

    history = np.zeros((args.nruns, args.nepisodes, 2))
    for run in range(args.nruns):
        # features = Tabular(env.observation_space.n)  # CHANGED
        # nfeatures, nactions = len(features), env.action_space.n  # CHANGED
        features = lambda x: np.nonzero(x)[0]  # CHANGED
        nfeatures, nactions = env.obs_dim, env.action_space  # CHANGED

        # The intra-option policies are linear-softmax functions
        option_policies = [SoftmaxPolicy(rng, nfeatures, nactions, args.temperature) for _ in range(args.noptions)]  # set of subpolicies
        if args.primitive:
        ...
        if args.primitive:
            option_terminations.extend([OneStepTermination() for _ in range(nactions)])  # if you want to have primitive actions alongside options

        # E-greedy policy over options
        #policy = EgreedyPolicy(rng, nfeatures, args.noptions, args.epsilon)
        policy = SoftmaxPolicy(rng, nfeatures, args.noptions, args.temperature)  # policy over options
        ...
        critic = IntraOptionQLearning(args.discount, args.lr_critic, option_terminations, policy.weights)  # critic for policy over options

        # Learn Qomega separately
        action_weights = np.zeros((nfeatures, args.noptions, nactions))
        action_critic = IntraOptionActionQLearning(args.discount, args.lr_critic, option_terminations, action_weights, critic)  # joint network for simultaneous critic for subpolicies

        # Improvement of the termination functions based on gradients
        termination_improvement= TerminationGradient(option_terminations, critic, args.lr_term)  # termination gradient fro all options
        ...
        for episode in range(args.nepisodes):
            if episode == 1000:  # TODO
                env.goal = rng.choice(possible_next_goals)
                print('************* New goal : ', env.goal)

            phi = features(env.reset())
            option = policy.sample(phi)  # given the state, sample the option: p(o | s)
            action = option_policies[option].sample(phi)  # given state and option, sample the action: p(a | s) = p(a | o, s) * p(o | s)
            critic.start(phi, option)  # initialize value computation over options
            action_critic.start(phi, option, action)  # initialize value computation over actions, conditioned on the option
            ...
            for step in range(args.nsteps):
                ...
                if option_terminations[option].sample(phi):  # if termination value is 1
                    option = policy.sample(phi)  # sample a new option
                    option_switches += 1
                    avgduration += (1./option_switches)*(duration - avgduration)
                    duration = 1

                action = option_policies[option].sample(phi)  # p(a | s) = p(a | o, s) * p(o | s)
